...
function newcell (keyseq, context, data) {
  return new Cell(keyseq, context, { codec: JsonCodec, data, format: ITEM_FORMAT })
}

class Shard {
  static async parse (string, { id, ...env }) {
    let keyseq = null

    let ctx = Context.create(CTX_PREFIX, { file: id })
    let keysCtx = ctx.add({ scope: SCOPE_KEYS })
    let indexCtx = ctx.add({ scope: SCOPE_INDEX })

    if (!string) {
      keyseq = new KeySequenceCipher(keysCtx, env)
      let index = newcell(keyseq, indexCtx).set([])
      return new Shard(ctx, keyseq, index, [])
    }
    ...
    keyseq = await KeySequenceCipher.parse(header.cipher, keysCtx, env)
    ...
    items = items.map((item, i) => {
      let context = ctx.add({ scope: SCOPE_ITEMS, path: paths[i] })
      return newcell(keyseq, context, item)
    })

    return new Shard(ctx, keyseq, index, items)
  }

  constructor (context, cipher, index, items) {
  ...
  async _update (path, options, fn) {
    return this._rwlock.write(async () => {
      let idx = await this._indexOf(path)
      if (idx < 0 && !options.create) return null

      let item = (idx >= 0) ? this._items[idx] : null
      let value = item ? await item.get() : null

      value = fn ? await fn(value) : null

      if (value === null) {
        await this._removeAt(idx)
      } else {
        await this._updateAt(idx, path, value)
      }
  ...
  async _updateAt (idx, path, value) {
    if (idx < 0) {
      idx = Math.abs(idx) - 1
      await this._insertAt(idx, path)
    }
  ...
  async _insertAt (idx, path) {
    ...
    let context = this._context.add({ scope: SCOPE_ITEMS, path })
