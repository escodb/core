...
class Shard {
  static async parse (id, string, cipher, verifier) {
    ...
    if (!string) {
      keyseq = new KeySequenceCipher({ ...ctx, scope: SCOPE_KEYS }, cipher, verifier)
      let index = newcell(keyseq, { ...ctx, scope: SCOPE_INDEX }).set([])
    ...
    keyseq = await KeySequenceCipher.parse(header.cipher, { ...ctx, scope: SCOPE_KEYS }, cipher, verifier)

    index = newcell(keyseq, { ...ctx, scope: SCOPE_INDEX }, index)
  ...
  async _getOr (path, init) {
    ...
    if (idx < 0) {
      ...
      let context = { shard: this._id, scope: SCOPE_ITEMS, path }
