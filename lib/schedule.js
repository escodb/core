...
class Schedule {
  ...
  _createOp (shard, deps, value, id = null) {
    for (let id of deps) {
      if (!this._operations.has(id)) {
        throw new ScheduleError(`unrecognised operation ID: '${id}'`)
      }
    }
...
class GroupHandle {
  ...
  * valuesWithIds () {
    ...
    let ops = this._schedule._getOperations(opIds)
    sortTopological(ops)
...
function sortTopological (items) {
  items.sort((a, b) => {
