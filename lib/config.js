...
class Config {
  ...
  _deriveKey (password) {
    ...
    return PasswordCipher.create({ ...params, password })
  }

  async _decrypt (scope, key) {
    let pwKey = await this._pwKey
    let context = Context.create(CTX_PREFIX, { file: SHARD_ID, scope })
...
async function buildInitialConfig (options) {
  ...
  let ctx = Context.create(CTX_PREFIX).add({ file: SHARD_ID })

  let newcell = (scope, value) => {
    let context = ctx.add({ scope })
