...
class KeySequenceCipher {
  static async parse ({ keys, state }, cipher, verifier, config = {}) {
    keys = keys.map((str) => {
      ...
      let cell = new Cell(cipher, KeyCodec, { data: key })
    ...
    return new KeySequenceCipher(cipher, verifier, config, keys, counters)
  }
  ...
  async encrypt (data, context = {}) {
    ...
    let enc = await cipher.encrypt(data, context)
  ...
  async decrypt (data, context = {}) {
    ...
    let cipher = new AesGcmCipher({ key })
    return cipher.decrypt(enc, context)
  }
  ...
  _generateNewKey (len) {
    ...
    let cell = new Cell(this._cipher, KeyCodec)
