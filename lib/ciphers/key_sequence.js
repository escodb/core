...
class KeySequenceCipher {
  static async parse ({ keys, state, mac }, context, env, config = {}) {
    context = context.prefix(CTX_PREFIX)
    let { cipher, verifier } = env

    keys = keys.map((str) => {
      let buf = Buffer.from(str, KEY_FORMAT)
      let [seq, key] = binaries.load([SEQ_TYPE, 'bytes'], buf)
      let ctx = context.add({ key: seq })
      let cell = new Cell(cipher, ctx, { codec: KeyCodec, data: key })
    ...
    let ctx = context.add({ keys: seqs, state })
    await verifier.verify(ctx, mac)

    let ids = keys.flatMap((key) => [`${key.seq}.msg`, `${key.seq}.blk`])
    let counters = await Counters.parse(state, ids)

    return new KeySequenceCipher(context, env, config, keys, counters)
  }

  constructor (context, { cipher, verifier }, config = {}, keys = null, counters = null) {
    this._context = context.prefix(CTX_PREFIX)
    this._cipher = cipher
  ...
  async serialize () {
    ...
    let ctx = this._context.add({ keys: seqs, state })
    let mac = await this._verifier.sign(ctx)
  ...
  async encrypt (data, context) {
    ...
    let ctx = context.prefix(CTX_PREFIX).add({ key: seq })
    let enc = await cipher.encrypt(data, ctx)
  ...
  async _createNewKey (nbytes) {
    ...
    let context = this._context.add({ key: seq })
    let cell = new Cell(this._cipher, context, { codec: KeyCodec })
