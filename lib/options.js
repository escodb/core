...
class Options {
  ...
  _buildRules (config, path = []) {
    if (typeof config.required === 'boolean' || typeof config.valid === 'function') {
      this._rules.set(path.join('.'), { path, ...config })
    } else {
      for (let [key, value] of Object.entries(config)) {
        this._buildRules(value, [...path, key])
      }
    }
  }
  ...
  _checkUnknownKeys (input, path = []) {
    ...
    if (this._rules.has(pathStr)) return

    if (typeof input === 'object' && input !== null) {
      for (let [key, value] of Object.entries(input)) {
        this._checkUnknownKeys(value, [...path, key])
      }
    } else {
      throw new ConfigError(`unrecognised option: '${pathStr}'`)
    }
  }
}
