...
class Store {
  ...
  async openOrCreate (createOpts) {
    try {
      ...
      try {
        return await this.create(createOpts)
      } catch (error) {
        if (error.code === 'ERR_EXISTS') {
          return this.openOrCreate(createOpts)
        } else {
          throw error
        }
      }
  ...
  async create (createOpts = {}) {
    let options = Options.merge(this._openOpts, createOpts)
    let config = await Config.create(this._adapter, options)
