...
class Executor {
  ...
  async _request (group) {
    ...
    for (let [id, op] of group.valuesWithIds()) {
      if (op.error) continue

      try {
        op.result = await op.fn(shard)
      } catch (error) {
        this._opFailed(group, id, op, error)
      }
    }
  ...
  _groupFailed (group, error) {
    for (let op of group.failed()) {
      op.reject(error)
    }
  }

  _opFailed (group, id, op, error) {
    for (let op of group.opFailed(id)) {
      op.error = error
      op.reject(error)
    }
  }
